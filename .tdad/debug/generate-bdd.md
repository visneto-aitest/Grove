...
**Target File (WRITE OUTPUT HERE):** `.tdad/workflows/agents/create-agent/create-agent.feature`
...
# BDD Generation: Create Agent

## Feature Description
POST /api/agents with {branchName, worktreePath}. Creates new git branch, creates git worktree, starts tmux session, and launches AI agent. Returns {id, branchName, worktreePath, tmuxSession, status} on success. Returns 400 with {error: 'Branch already exists'} if branch exists.
...
## Your Task
Write the Gherkin specification for **Create Agent**.
